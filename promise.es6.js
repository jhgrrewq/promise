(function(global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' 
    ...
    : typeof defind === 'function' && typeof defind.amd !== 'undefined' 
        ? define(factory) 
    ...
    class Promise {
        constructor(fn) {
            ...
            this.fulfilledCallbackList = [] // 存储成功回调函数数组
            this.rejectedCallbackList = [] // 存储失败回调函数数组
            ...
            const resolve = value => {
                ...
                if (this.state === PENDING) {
                    ...
                    this.fulfilledCallbackList.forEach(cb => cb(value))
                }
            }
            ...
            const reject = reason => {
                if (this.state === PENDING) {
                    ...
                    this.value = reason // 保存失败值

                    this.rejectedCallbackList.forEach(cb => cb(reason))
                }
            }
        ...
        then(onFulfilled, onRejected) {
            ...
            if (this.state === PENDING) {
                ...
                newPromise = new Promise((resolve, reject) => {
                    ...
                    // 同上
                    this.rejectedCallbackList.push(reason => {
                        setTimeout(() => {
                            try{
            ...
            if (this.state === FULFILLED) {
                ...
                newPromise = new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => {
                        try {
                            const newValue = fulfilledCallback(this.value)
                            resolveValue(newValue, resolve, reject)
                        } catch(err) {
                            reject(err)
                        }
            ...
            if (this.state === REJECTED) {
                ...
                newPromise = new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => {
                        try{
                            const newReason = rejectedCallback(this.value)
                            resolveValue(newReason, resolve, reject)
                        } catch(err) {
                            reject(err)
                        }
        ...
        static race(values) {
            ...
            return new Promise((resolve, reject) => {
                values.forEach(value => {
                    ...
                    // 将新 Promise 对象的 resolve reject 传递给解析后的 then
